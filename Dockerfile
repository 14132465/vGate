...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o vgate ./cmd/gate
...
# 将编译好的二进制文件从 builder 复制过来  COPY 源 ，目标
COPY --from=builder /app/vgate /application/vgate

# 如果项目需要配置文件，也一并复制  COPY 源 ，目标
...
#一定要设置工作目录   ENTRYPOINT ["/application/vGate"] 是找不到配置文件的
WORKDIR /application
# 运行
ENTRYPOINT ["./vgate"]